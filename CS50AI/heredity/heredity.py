...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    probability = 1
    for person in people:
        # Calculating joint probabiltiy of a person (parent)
        ...
        # Calculating joint probabiltiy of a person (child)
        else:
            ...
            # Calculating child probability
            if person in one_gene:
                if person in have_trait:
                    probability *= PROBS["trait"][1][True] * (prob_m * (1 - prob_f) + (1 - prob_m) * prob_f)
                else:
                    probability *= PROBS["trait"][1][False] * (prob_m * (1 - prob_f) + (1 - prob_m) * prob_f)
            elif person in two_genes:
                if person in have_trait:
                    probability *= PROBS["trait"][2][True] * prob_m * prob_f
                else:
                    probability *= PROBS["trait"][2][False] * prob_m * prob_f
            else:
                if person in have_trait:
                    probability *= PROBS["trait"][0][True] * (1 - prob_m) * (1 - prob_f)
                else:
                    probability *= PROBS["trait"][0][False] * (1 - prob_m) * (1 - prob_f)
    return probability
