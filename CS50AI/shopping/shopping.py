...
def load_data(filename):
    ...
    """
    integer_indexes = [0, 2, 4, 11, 12, 13, 14]
    ...
    with open(filename) as file:
        ...
        next(reader)
        for row in reader:
            evidences.append(row[:len(row) - 1])
            labels.append(1) if row[len(row) - 1] == "TRUE" else labels.append(0)
    for evidence in evidences:
        for i in range(len(evidence)):
            if i in integer_indexes:
                evidence[i] = int(evidence[i])
            elif i in float_indexes:
                evidence[i] = float(evidence[i])
            elif i == 10:
                evidence[i] = months.index(evidence[i])
            elif i == 15:
